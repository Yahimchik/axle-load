...
import com.mehatronics.axle_load.service.impl.GattReadServiceImpl;
import com.mehatronics.axle_load.service.impl.GattWriteServiceImpl;
...
public class BluetoothGattCallbackHandler extends BluetoothGattCallback {
    ...
    private final GattReadServiceImpl gattReadServiceImpl;
    private final GattWriteServiceImpl writeProcessor;
     ...
     * @param gattReadServiceImpl processes GATT read operations and parsing
    ...
    public BluetoothGattCallbackHandler(
            ...
            GattReadServiceImpl gattReadServiceImpl,
            GattWriteServiceImpl writeProcessor,
            GattDataParser gattDataParser,
            CommandStateHandler stateHandler
    ) {
        this.connectionManager = connectionManager;
        this.gattReadServiceImpl = gattReadServiceImpl;
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        if (isStatusOk(status, BluetoothGatt.GATT_SUCCESS)) {
            ...
            gattReadServiceImpl.readAllCharacteristics(gatt);
        }
    }
    ...
    public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        if (isStatusOk(status, BluetoothGatt.GATT_SUCCESS)) {
            gattReadServiceImpl.handleRead(gatt, characteristic);
            if (!gattReadServiceImpl.isReadingAll()) {
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        if (isStatusOk(status, BluetoothGatt.GATT_SUCCESS)) {
            if (characteristic.getUuid().equals(WRITE_CHARACTERISTIC_DPS)) {
                gattReadServiceImpl.readNextAfterWrite(gatt);
            }
        }
    }
    ...
    public int getTablePage() {
        return gattReadServiceImpl.getTablePage();
    }
    ...
    public LiveData<DeviceDetails> getDeviceDetailsLiveData() {
        return gattReadServiceImpl.getDeviceDetailsLiveData();
    }
    ...
    public void setDeviceDetailsLiveData(DeviceDetails details) {
        gattReadServiceImpl.setDeviceDetailsLiveData(details);
    }
    ...
    public LiveData<SensorConfig> getSensorConfigureLiveData() {
        return gattReadServiceImpl.getSensorConfigureLiveData();
    }
    ...
    public void clearDetails() {
        gattReadServiceImpl.clearDetails();
    }
    ...
    public void resetState() {
        ...
        gattReadServiceImpl.updateState(false);
    }
    ...
    public void setConfigurationSaved(boolean value) {
        gattReadServiceImpl.setConfigurationSaved(value);
    }
    ...
    public boolean isConfigurationSaved() {
        return gattReadServiceImpl.isConfigurationSaved();
    }
    ...
    public void setTableSaved(boolean value) {
        gattReadServiceImpl.setTableSaved(value);
    }
    ...
    public boolean isTableSaved() {
        return gattReadServiceImpl.isTableSaved();
    }
    ...
    public void rereadCalibrationTable() {
        ...
        gattReadServiceImpl.rereadCalibrationTable();
    }
    ...
    public void saveConfiguration() {
        var sensorConfig = gattReadServiceImpl.getSensorConfigureLiveData().getValue();
        if (sensorConfig == null) {
            return;
        }
        gattDataParser.setConfigureSettings(sensorConfig, writeProcessor.getBuffer());
        gattReadServiceImpl.setConfigurationSaved(false);
    }
    ...
    public void saveTableToSensor() {
        ...
        gattReadServiceImpl.setTablePage(writtenPage);

        if (getTablePage() < 0) {
            gattReadServiceImpl.setTableSaved(false);
    ...
    private void updateStateAfterConnect() {
        gattReadServiceImpl.updateState(true);
